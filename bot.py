...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "cart:view":
            await shopping_cart.view_cart(update, context)
            return
        ...
        if data == "cart:checkout":
            await shopping_cart.checkout_cart(update, context)
            return

        if data == "cart:paynow":
            await shopping_cart.paynow_cart(update, context)
            return

        if data == "cart:confirm_payment":
            await shopping_cart.confirm_payment(update, context)
            return

        if data == "cart:cancel":
            await update.callback_query.edit_message_text("❌ Checkout cancelled.")
            return
