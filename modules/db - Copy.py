...
async def get_seller_products(user_id: int):
    ...
    Used by UI to decide whether to show seller menu.
    """
    query = """
        SELECT p.*, 
...
async def admin_get_users_paginated(page: int, size: int):
    offset = (page - 1) * size
    async with pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT * FROM users
...
async def admin_get_products_paginated(page: int, size: int):
    offset = (page - 1) * size
    async with pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT * FROM product
